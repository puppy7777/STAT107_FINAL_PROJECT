...
```{r}
disaster_insurance <- lm(Disaster_Cost ~ Property_Insurance_Index, data = all_insurance)
summary(disaster_insurance)

log_disaster_insurance <- lm(log(ModifiedDisaster) ~ Property_Insurance_Index, data = all_insurance)
summary(log_disaster_insurance)
```

```{r}
disaster_all <- lm(Disaster_Cost ~ All_Insurance_Index, data = all_insurance)
summary(disaster_all)

log_disaster_all <- lm(log(ModifiedDisaster) ~ All_Insurance_Index, data = all_insurance)
...
```{r}
year_delay <- lm(Property_Insurance ~ diff_1_year, data = delayed_damage)
summary(year_delay)
```

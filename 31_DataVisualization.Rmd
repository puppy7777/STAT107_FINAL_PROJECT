...
```{r}
#these are our graphs of disaster costs over time, first is linear scale second is log scale - not really any sort of correlation visible except maybe a slight upwards correlation for log scale
...
above shows that disaster costs are not correlated with time, what about
disaster costs leading to increase in property insurance?
...
We checked both linear disaster cost as well a log(disaster cost), but
neither had any correlation. We also check overall insurance prices,
although I personally doubt it will be different
...
We have determined there is no linear correlation beteween insurance
prices and the scale of natural disasters, but what if insruance prices
just keep going up with time?
...
this does seem to be the case. We can also check to see if there is a
delay in insurance prices increasing as a cuase of large natural
disasters - we can check 1 month 6 month and 12 month delays
...
diff_6_month <- c()
for (num in 1:(nrow(delayed_damage))) {
  diff_6_month <- append(diff_6_month, (delayed_damage$Property_Insurance[num+6]/ delayed_damage$Property_Insurance[num]))
}
...
we also run 1 year delayed with the log of the damage done, but to no
results. we could run this for the other time groups but that feels
excessive-it seems pretty clear there is no correlation.

```{r}
plot(diff_1_year ~ ModifiedDamage, data = delayed_damage, pch = 20, log = "x", xlab = "Natural Disaster Damage During a Given Month", ylab = "Relative Price of Property Insurance 1 Year Later")
